...
;; it in an emacs-concistent manner. There is also a quick-search
;; function that display an immediate help if the current word exists,
;; without generating any intermediate document.
;;
...
;;      (global-set-key '[f12] 'javahelp-quick-search)
;;  or  (global-set-key '[f12] 'javahelp-search-emacs-flavour)
;;  You can also bind the three methods to three different keys
;;  to use them all.
;;
...
;;
;; TO DO: 
;;   * improve the database creation by 
;;      - making it fast
;;      - reducing the noise
;;   * search for the class of a variable, so that calling
;;     help for a variable which is declared as "Vector" in the
;;     current scope will call help for "vector".
;;   * Bug fixing ?
;;
;;
...
(defvar javahelp-temp-dir "d:/temp/"
...
(defvar javahelp-database nil
  "Contain the BIG lexdfa database of javahelp.
Don't initialize it yourself."  )

(defvar javahelp-database-dirs nil
  "A vector of directories. Don't use it by yourself.")
  ...
  (if (null alist)
    ...
    (let* ( (pair (javahelp-assoc key alist)) )
	  ...
	  (append (list (list key value))
		  alist)
	(progn (javahelp-alist-add-last pair value)
	       alist)))))
...
(defun javahelp-quick-search ()
  "Search the current word, and display it immediatly (in the browser).
No html document is generated, and the first result found is displayed."
  (interactive)
  (if (null (javahelp-help-for-word (javahelp-short-class-name (javahelp-current-java-word))))
