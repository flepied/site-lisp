...
(defun header-file-name ()
 ...
 (insert header-prefix-string "File            : " (file-name-nondirectory (buffer-file-name)) "\n"))
...
(defun update-the-file-name ()
      (delete-and-forget-line)
      (insert (format "%s" (file-name-nondirectory (buffer-file-name))))
      )
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;(add-hook command-line-hooks 'set-header-required-mode)
...
(progn
  ...
  (register-file-header-action "[ \t]File[ \t]*: "  'update-the-file-name)
