...
;;   Created: June 1994
;;   Last modified: 1997/06/19 13:06:16
;;   Version: 2.25
...
;; 1997/06/19 13:06:16|2.25|~/misc/mode-compile.el.Z|
;;
...
;;  modified buffers could be automaticaly saved if variable
...
;; ===========
;;
;;   cl-macs is now loaded from here.
;;   efs-ftp-path is now called to check remote compilations.
;;
...
(defvar mode-compile-exe-file-ext
  (cond
   ((memq system-type '(ms-dos emx windows-95 windows-nt)) ".exe")
   (t ""))
...
(defvar mode-compile-dir-separator-char
  (cond
   ...
   ((memq system-type '(ms-dos emx windows-95 windows-nt)) "\\")
...
;;;###autoload
(defconst mode-compile-version "2.25"
  "Current version of mode-compile package.
 
mode-compile.el,v 2.25 1997/06/19 13:06:16 boubaker Exp
...
(defun mc--set-remote-cmd (remote-host &optional username)
...
(defun mode-compile-submit-bug-report ()
  "*Submit via mail a bug report on mode-compile v2.25."
  (interactive)
  (and
...
Bound on \\[mode-compile]." 
  ...
  (if remote-host
    ...
    (setq mc--efs-path-list (and (fboundp 'efs-ftp-path) (efs-ftp-path (buffer-file-name))))
