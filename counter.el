...
;; insert series of numbers into text.  You'll need to bind that function to
;; a key; I suggest C-x #, which is currently unused in GNU Emacs.  Detailed
;; instructions on how to do that key binding, and on everything else, are
;; below, but here's a summary of what that binding will allow.  Every time
;; you type C-x #, counter inserts a number into the buffer, then increments
;; the value it's up to, so the next C-x # inserts the next number.
;; You can type C-u C-x # to initialize the starting number, the increment,
;; and the format of the insertion (leading or trailing spaces, for example,
;; or other decoration).  C-u 5 C-x # will insert five numbers; that's the
;; fastest way to insert a series of numbers if you want them all together in
;; the text, separated by nothing but the format decoration.  But you can
;; also use C-x # in keyboard macros, allowing you to do things like insert
;; columns of numbers.  There is also a facility to allow you to renumber a
;; possibly-noncontiguous column, such as occurs with numbered paragraphs.
...
;; Then you'll need to bind the `counter' function.  Remember that C-x # is
;; not reserved for users in Emacs; only C-c followed by letters are so
;; reserved, so if you choose C-x # you may someday need to choose something
;; else.  I still recommend C-x # for now.  This code in your .emacs
;;   (global-set-key "\C-x#" 'counter)
...
;; C-u 4 C-x # to insert a series of 4 numbers.  Then C-u 3 C-x # will insert
;; 3 more numbers without resetting the counter; it doesn't get reset until
;; you reinitialize it with C-u C-x #.

;; C-u C-x # prompts for 3 values: the number to start counting from, the
...
;; *replace* them.  Invoking `counter' as C-u - C-x # causes it to search the
...
;;   C-x #       ;; counter
;; to mark each word and run C-u - C-x # on it; if the word contains a
...
;; The `counter-target' variable isn't modified by C-u C-x # because most
;; users will never read this far in the documentation and won't know what it
;; is.  You can modify it with M-x set-variable.

;; If your formatting needs are too complex for an Emacs regex, you may need
...
that region for the first string matching the regex in `counter-target', and
