...
;; Last Modified On: Tue Oct 10 13:47:55 1995
;; Update Count    : 59
...
(defvar lisp-code-directory
 "/anonymous@archive.cis.ohio-state.edu:pub/gnu/emacs/elisp-archive/LCD-datafile.Z"
...
;  (require 'ange-ftp)
;  (require 'crypt)
  (file-exists-p (concat "/ftp@" file)))

(defun remote-find-file (file)
  ;; Remote-fetch a file
;  (require 'ange-ftp)
;  (require 'crypt)
  (find-file-noselect (concat "/ftp@" file)))
...
;;   (if (string-match "\\.Z$" lisp-code-directory)
;;       (require 'crypt))
