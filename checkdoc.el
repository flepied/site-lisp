...
;; X-RCS: $Id: checkdoc.el,v 1.65 1998/05/15 02:24:46 zappo Exp $
...
;;
;;   The text that follows the `error' command is also checked.  The
;; documentation for `error' clearly states some simple style rules to
;; follow which checkdoc will auto-fix for you.
;;
...
(defun checkdoc-current-buffer (&optional take-notes)
  "Check current buffer for document, commaent, error style, and rogue spaces.
...
warnings buffer, otherwise it stops after the first error."
  ...
  (let ((checkdoc-spellcheck-documentation-flag
	...
	(checkdoc take-notes)
	(checkdoc-error-text take-notes)
...
space at the end of each line."
  ...
  (save-excursion
	...
	(if msg (if no-error (message msg) (error msg))
	  (setq msg (checkdoc-error-text-search beg end))
	  (if msg (if no-error (message msg) (error msg))
...
(defvar checkdoc-minor-keymap
  (let ((map (make-sparse-keymap))
    ...
    (define-key pmap "m" 'checkdoc-error-text)
...
(if checkdoc-minor-keymap
	...
	buffer-file-name]
       ["Check error message text" checkdoc-error-text t]
...
;;
(defun checkdoc-error-text (&optional take-notes)
  "Scan the buffer for occurances of the error function, and verify text.
Optional argument TAKE-NOTES causes all errors to be logged."
  (interactive "P")
  (if take-notes (checkdoc-start-section "checkdoc-error-text"))
  (let ((p (point))
	(e (checkdoc-error-text-search)))
...
(defun checkdoc-error-text-search (&optional beg end)
...
The default boundary is the entire buffer."
  (let ((e nil))
    ...
    (goto-char beg)
    (while (and (not e) (re-search-forward "(\\s-*error\\s-+" end t))
      (if (looking-at "\"")
	  (setq e (checkdoc-error-text-engine))))
    e))
  
(defun checkdoc-error-text-engine ()
  "Return an error string if errors are found in any strings passed to error.
    ...
    (or
     ...
     (if (and (save-excursion (forward-char 1)
	      ...
	      (not (checkdoc-autofix-ask-replace (match-beginning 0)
						 (match-end 0)
						 "Capitalize your error text?"
						 (capitalize (match-string 0))
						 t)))
	 "Error messages should start with a capitol letter"
       nil)
     (if (and (save-excursion (forward-sexp 1)
			      (forward-char -2)
			      (looking-at "\\."))
	      (not (checkdoc-autofix-ask-replace (match-beginning 0)
						 (match-end 0)
